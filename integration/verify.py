...
def module_teardown(user_domain, app_dir, data_dir, platform_data_dir):
    ...
    run_ssh(user_domain, 'ls -la {0}/roundcube/config/ > {1}/log/roundcube.config.ls.log'.format(app_dir, data_dir), password=LOGS_SSH_PASSWORD, throw=False)
...
def imap_php(user_domain, platform_data_dir, app_dir, name, data_dir):
    ...
    run_ssh(user_domain, 'cp {0}/config/roundcube/config.inc.php /'.format(data_dir), password=DEVICE_PASSWORD)
    run_scp('{0}/php.ssl.imap.test.php root@{1}:/'.format(DIR, user_domain, platform_data_dir), password=DEVICE_PASSWORD)
