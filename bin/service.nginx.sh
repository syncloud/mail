...
start)
    ...
    ${DIR}/nginx/sbin/nginx -t -c ${SNAP_COMMON}/config/nginx/nginx.conf -p ${DIR}/nginx -g 'error_log '${SNAP_COMMON}'/log/nginx_error.log warn;'
    ...
    ;;
post-start)
    timeout 60 /bin/bash -c 'until [ -S '${SNAP_COMMON}'/web.socket ]; do echo "waiting for ${SNAP_COMMON}/web.socket"; sleep 1; done'
    ;;
reload)
